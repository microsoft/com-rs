...
pub fn expand_co_class(input: &ItemStruct, attr_args: &AttributeArgs) -> TokenStream {
    ...
    out.push(com_struct::generate(&base_interface_idents, input).into());

    out.push(com_struct_impl::generate(&base_interface_idents, input).into());

    out.push(co_class_impl::generate(input).into());

    out.push(iunknown_impl::generate(&base_interface_idents, input).into());
